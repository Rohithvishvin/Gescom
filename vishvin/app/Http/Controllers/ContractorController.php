...
$data = [
    ...
    public function rejected_reports()
    {
        ...
        $meter_mains = Meter_main::join('admins', 'admins.id', '=', 'meter_mains.created_by')
                                ->where('qc_status', 2)
                                ->orWhere('so_status', 2)
                                ->orWhere('aee_status', 2)
                                ->orWhere('aao_status', 2)
                                ->select('meter_mains.*', 'admins.created_by as contractor_id')
                                ->orderByDesc('meter_mains.id') // Order by 'id' in descending order
                                ->get();
        // dd($meter_mains);
    ...
    /*public function index()
    {
        ...
        return view('contractors.index', ['data' => $data]);
    } */
    ...
    $today_fe_installed_results = DB::table('admins as a')
    ->join('meter_mains as m', 'a.id', '=', 'm.created_by')
    ->select('a.name', 'm.created_by', DB::raw('COUNT(*) as count'))
    ->where('a.type', 'field_executive')
    ->whereNotNull('m.serial_no_old')
    ->whereNotNull('m.serial_no_new')
    ->where('a.created_by', session('rexkod_vishvin_auth_userid'))
    ->whereDate('m.created_at', Carbon::today())
    ->groupBy('a.name', 'm.created_by')
    ->get();
        ...
        $data = [
            'contractor_inventories_by_id' => $contractor_inventories_by_id,
            'get_field_executives' => $get_field_executives,
          'today_fe_installed_results' => $today_fe_installed_results
        ];
