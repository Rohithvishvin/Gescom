            ...
            <div class="card-body">
                ...
                <table class="table" style="table-layout: fixed;">
                    ...
                    <tbody>
                        ...
                        @foreach ($contractor_inventories as $contractor_inventory)
                            ...
                            <tr>
                                ...
                                $diff = count($nums);
                                $total_meters = $total_meters + $diff;
                                if(!$contractor_inventory->unused_meter_serial_no == null){
                                    $unused_meters = $unused_meters+ count(explode(',', $contractor_inventory->unused_meter_serial_no));
                                }else {
                                    $unused_meters = $unused_meters+0;
                                }
                                if(!$contractor_inventory->used_meter_serial_no == null){
                                    $used_meters = $used_meters+ count(explode(',', $contractor_inventory->used_meter_serial_no));
                                }else {
                                    $used_meters = $used_meters+0;
                                }

                                ?>
                                <td>{{ $diff }}</td>
                                <td>{{ $contractor_inventory->unused_meter_serial_no }}</td>
                                <td>{{!$contractor_inventory->unused_meter_serial_no == null ? count(explode(',', $contractor_inventory->unused_meter_serial_no)) : 0}}</td>
                                <td>{{ $contractor_inventory->used_meter_serial_no }}</td>
                                <td>{{!$contractor_inventory->used_meter_serial_no == null ? count(explode(',', $contractor_inventory->used_meter_serial_no)) : 0}}</td>

                                <?php
                                $created_by_detail = Admin::where('id', $contractor_inventory->created_by)->first();
                                ?>
                            
                                <td>{{ isset($created_by_detail->name) ? ucwords($created_by_detail->name) : 'N/A' }}</td>
                    ...
                    <footer>
                        <tr>
                            <td></td>
                            <td colspan="3"><h4>Total: {{$total_meters}}</h4></td>
                            <td colspan="3"><h4>Used: {{$used_meters}}</h4></td>
                            <td colspan="3"><h4>Unused: {{$unused_meters}}</h4></td>
                        </tr>
                    </footer>
                </table>
            </div>
