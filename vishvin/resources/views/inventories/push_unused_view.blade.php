                ...
                <div class="col-xl-12 col-md-12 dash-xl-100 dash-lg-100 dash-39">
                    <div class="card ongoing-project recent-orders">
                        ...
                        <div class="card-body pt-0">
                            <form id="search-fault-serial-id-form" autocomplete="off" class="needs-validation" action="/inventories/Push_serial_number_unused" method="POST"  enctype="multipart/form-data">
                              @csrf
                            <div class="row">
                                    <div class="col-4">
                                        <label class="form-label" for="product-title-input">Push serial Number Unused  ?
                                    ...
                                    <!-----<div class="col-4">
                                        <label class="form-label" for="product-title-input">Replaced Meter Serial Number
                                            <span class="mandatory_star">
                                            *<sup><i>required</i></sup>
                                        </span>
                                        </label>
                                  <input type="text" class="form-control" id="new_meter_serial_no" name="new_meter_serial_no" value="" placeholder="Enter New Meter Serial No">
                                    ...
                                    <div class="col-4">
                                      ...
                                      // print_r("----------------------");
                                                if (!empty($errors)) {
                                                    // Assuming $errors is an array containing error messages
                                                     // dd($errors);
                                                    // Check if faulty meter serial number data exists and if it's empty or wrong
                                                ...
                                                ?>
                                                        ...
                                                        <!---------------   faulty meter validation status ----->
                                                            <!---@if(isset($faulty_meter_inventory_status) && ($faulty_meter_inventory_status == "used" || $faulty_meter_inventory_status == "unused"))
                                                            @if(isset($faulty_meter_status) && ($faulty_meter_status == "Exist" || $faulty_meter_status == " "))
                                                            <div class="row mt-4">
                                                                        <div class="col-12">
                                                                            <h3 class="bg-success">Input Meter Serial Number Might be faulty</h3>
                                                                        </div>
                                                            </div> 
                                                            @endif
                                                            @endif----->
                                                           ...
                                                           <!------ @if(isset($faulty_meter_inventory_status) && $faulty_meter_inventory_status === "")
                                                                        <div class="row mt-4">
                                                                            <div class="col-12">
                                                                                <ul>
                                                                                    <li class="bg-warning">Error: Faulty meter serial number has invalid contractory inventory status.</li>
                                                                                </ul>
                                                                            </div>
                                                                        </div>
                                                                    @endif
                                                            ...
                                                            @if(isset($data["error"]["data"]) && $data["error"]["data"] === true)
                                                                        <div class="row mt-4">
                                                                            <div class="col-12">
                                                                                <h3 class="bg-danger">Invalid Data</h3>
                                                                                @if(isset($data["faulty_meter_serial_no"]) && !empty($data["faulty_meter_serial_no"]))
                                                                                    <ul>
                                                                                        @foreach($data["faulty_meter_serial_no"] as $key => $value)
                                                                                            <li>{{ $key }}: {{ $value }}</li>
                                                                                        @endforeach
                                                                                    </ul>
                                                                                @endif
                                                                                @if(isset($data["new_meter_serial_no"]) && !empty($data["new_meter_serial_no"]))
                                                                                    <ul>
                                                                                        @foreach($data["new_meter_serial_no"] as $key => $value)
                                                                                            <li>{{ $key }}: {{ $value }}</li>
                                                                                        @endforeach
                                                                                    </ul>
                                                                                @endif
                                                             ...
                                                             @endif
                                                                    ...
                                                                    @if(isset($data["error"]["message"]))
                                                                        <div class="row mt-4">
                                                                            <div class="col-12">
                                                                                <h3 class="bg-danger">{{ $data["error"]["message"] }}</h3>
                                                                            </div>
                                                                        </div> 
                                                                    @endif--->
                                                                    ...
                                                                    <!-------------@if(isset($faulty_meter_inventory_status) && isset($faulty_meter_status) && ($faulty_meter_inventory_status == " " || $faulty_meter_status == " " || $faulty_meter_status == null)) 
                                                                        <div class="row mt-4">
                                                                            <div class="col-12">
                                                                                <h3 class="bg-warning">Wrong meter Serial Number</h3>
                                                                            </div>
                                                                        </div> 
                                                                        @endif--->

                                                            <!-----@if(isset($new_meter_mains_inventory_status) && $new_meter_mains_inventory_status == "unused")
                                                                    @if(isset($new_meter_mains_ststus) && $new_meter_mains_ststus == " ")
                                                                        <div class="row mt-4">
                                                                            <div class="col-12">
                                                                                <h3 class="bg-success">New Meter Serial Number is Valid</h3>
                                                                            </div>
                                                                        </div> 
                                                                    @endif
                                                            @endif--->
