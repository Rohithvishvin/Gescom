...
<style>
    .mandatory_star{
        ...
        <title>ANX-1 Abstract report (Division:{{$data['division'] ? $data['division']: ''}},Sub-division:{{$data['subdivision'] ? $data['subdivision']: ''}},Section:{{$data['section'] ? $data['section']: ''}})</title>
                    ...
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                ...
                                <li class="breadcrumb-item active">ANX-1 Abstract report</li>
                            </ol>
                        </div>
                ...
                <div class="col-xl-12 col-md-12 dash-xl-100 dash-lg-100 dash-39">
                    <div class="card ongoing-project recent-orders">
                        <div class="card-header border-0 align-items-center d-flex">
                            <h4 class="card-title mb-0 flex-grow-1">ANX-1 Abstract report</h4>
                        </div>
                                ...
                                <table class="table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Target as per PERT chart (count)</th>
                                            <th>Actual Replaced(count)</th>
                                            <th>Replacement Achieved(%)</th>
                                            <th>BMR prepared(count)</th>
                                            <th>BMR updated in BMR(count)</th>
                                            <th>BMR Achieved(%)</th>
                                            {{-- <th>Remarks</th> --}}
                                    ...
                                    <tbody>
                                        ...
                                        @endphp
                                        @foreach ($data['annexure_1'] as $annexure_1)
                                        @php
                                        $count++;
                                        // $get_consumer_detail = Consumer_detail::where('account_id', $meter_main->account_id)->first();
                                        // $formattedDate = Carbon::createFromFormat('Y-m-d H:i:s', $annexure_1->created_at)->format('d-m-Y');
                                        @endphp
                                        <tr>
                                            <td>{{$count}}</td>
                                            <td>{{$annexure_1->division}}</td>
                                            <td>{{$annexure_1->sd_pincode}}</td>
                                            <td>{{$annexure_1->so_pincode}}</td>
                                            <td>{{$annexure_1->target_to_achieve}}</td>
                                            <td>{{$annexure_1->replaced_count}}</td>
                                            @php
                                            if(!$annexure_1->replaced_count == 0){
                                                $replacement_perc = round(($annexure_1->target_to_achieve - $annexure_1->replaced_count)/$annexure_1->target_to_achieve, 2);
                                            }else {
                                                $replacement_perc = 0;
                                            }
                                            @endphp
                                            <td>{{$replacement_perc}}</td>
                                            <td>{{$annexure_1->bmr_prepared}}</td>
                                            <td>{{$annexure_1->bmr_updated_count}}</td>
                                            @php
                                            if(!$annexure_1->bmr_prepared == 0){
                                                $bmr_achieved = round(($annexure_1->bmr_prepared - $annexure_1->bmr_updated_count)/$annexure_1->bmr_prepared, 2);
                                            }else {
                                                $bmr_achieved = 0;
                                            }
                                            @endphp
