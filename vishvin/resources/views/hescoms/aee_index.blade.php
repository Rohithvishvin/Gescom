        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="radio-toolbar col-12">
                    <h4>Sub Division</h4>
                    @foreach ( $data['sub_division'] as $section)
                        <input type="hidden" id="divisionCode" name="division" value='{{ $section->div_code}}'>
                        <div class="col-lg-3">
                            <input type="radio" id="subDivisionCode" name="subDivisionCode" value='{{ $section->sd_code}}'>
                            <label for="subDivisionCode">{{ $section->sub_division }}</label>
                        </div>
                    @endforeach
                </div>
            </div>
            <!-- Sub division start -->
            <div class="row">
                <div id="section" style='display: block' class="section-code sub"></div>
            </div>
            <div class="overlay">
...
<script>
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    resp.sub_division_code_res.forEach(function (e) {
                        var radioBtn = $('<input type="radio" id="' + e.sd_code + 'Code" name="subDivisionCode" value="' + e.sd_code + '"><label  class="me-lg-3" for="' + e.sd_code + 'Code">' + e.sub_division + '</label>');
        ...
        function subDivisionCode() {
            ...
            $.ajax({
                type: 'GET',
                url:'/hescoms/count/'+divisionCode+'/'+ this.value,
                datatype: 'json',
                success: function(resp){
                    $('#section').empty();
                    $('#section').append('<h3>Section Code</h3>')
                    resp.section_code_res.forEach(function(e) {
                    var radioBtn = $('<input type="radio" id="'+ e.so_code+'Code" name="sectionCode" value="'+ e.so_code+'"><label  class="me-lg-3" for="'+e.so_code+'Code">'+e.section_office+'</label>');
